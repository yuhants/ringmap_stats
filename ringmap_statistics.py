...
output_directory = '/home/yuhants/ringmap_stats/rev03/'
...
# Load the JPL ephemeris DE421 for Skyfield (covers 1900-2050)
...
def plot_ringmap(ringmap, fig=None, pos=None, title = 'Ringmap', extent=None, cmap='RdBu', vmin=-1, vmax=1, ytick=True, ylabel=True, xtick=True, xlabel=True, cbar=True):
...
class RingMap:
    ...
    def get_masked_map(self, freq, median_subtract = False):
        ...
        """
        masked_map = np.ma.array(self.ringmap[freq], mask = self.get_ra_mask(freq), copy=True)
        if median_subtract:
            # Median subtract along rows
...
def main(freqs, debug = False):
    ...
    for day in days:
        ...
        for i, freq in enumerate(freqs):
            ...
            stats = ringmap_stats(template_map, day_map, freq, plot = False)
